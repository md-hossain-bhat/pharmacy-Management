...
$(document).ready(function() { 
    $('#pur_datepicker').datepicker();
    $('#pur_datepicker').on("changeDate", function() {
        $('#my_hidden_input').val(
...
$(document).ready(function() {
    ...
    $('#exdatepicker').on("changeDate", function() {
        $('#my_hidden_input').val(
            $('#datepicker').datepicker('getFormattedDate')
        );
    });
...
<script>
  $(document).ready(function (){
        ...
        var purchase_date = $('#pur_datepicker').val();
        var date = $('#datepicker').val();
        var ex_date = $('#exdatepicker').val();
        var purchase_no = $('#purchase_no').val();
        var supplier_id = $('#supplier_id').val();
        var company_id = $('#company_id').val();
        var name = $('#company_id').find('option:selected').text();
        ...
        if(company_id ==''){
          $.notify("company is require",{globalPosible: 'top right',className: 'error'});
          return false;
        }
        if(category_id ==''){
          $.notify("company is require",{globalPosible: 'top right',className: 'error'});
        ...
        if(brand_id ==''){
          $.notify("brand is require",{globalPosible: 'top right',className: 'error'});
          return false;
        }
        if(product_id ==''){
...
<script>
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          $.each(data,function(key,v){
            html +='<option value="'+v.company_id+'">'+v.company.name+'</option>';
          });
    ...
    $(document).on('change','#company_id',function(){
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          $.each(data,function(key,v){
            html +='<option value="'+v.id+'">'+v.category.name+'</option>';
          });
  ...
  $(function() {
    $(document).on('change','#category_id',function(){
      ...
      $.ajax({
        ...
        success:function(data){
          var html ='<option value="">Select Genaric</option>';
          $.each(data,function(key,v){
    ...
    $(document).on('change','#brand_id',function(){
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          $.each(data,function(key,v){
            html +='<option data-buying_prod_price="'+v.unit_price+'" value="'+v.product.id+'">'+v.product.product_name+'</option>';
          });
  ...
  $(document).ready(function (){
        ...
        var product_id = $('#product_id').val();
        var product_name = $('#product_id').find('option:selected').text();
        var buying_prod_price = $('#product_id').find('option:selected').data('buying_prod_price');
        ...
        var data = {
          ...
          product_id:product_id,
          product_name:product_name,
          buying_prod_price:buying_prod_price
        };
        var html = template(data);
        $("#addrow").append(html);
        });
        ...
        $(document).on('keyup click','.selling_qty',function(){
          var selling_qty = $(this).closest("tr").find("input.selling_qty").val();
          var buying_prod_price = $(this).closest("tr").find("input.buying_prod_price").val();
          var sub_total = buying_prod_price * selling_qty;
          $(this).closest("tr").find("input.buying_sub_price").val(sub_total);
        });
        ...
        function totalAmountPrice(){
          ...
          $('#estamited_amount').val(sum);
          var sum2 =0;
          $(".buying_sub_price").each(function(){
            var value = $(this).val();
            if(!isNaN(value) && value.length !=0){
